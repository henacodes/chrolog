    ...
    <div className="flex flex-col gap-5 w-full overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 min-w-0">
          {appIcon && (
            <img src={appIcon} className="w-7 h-7 object-contain shrink-0 rounded-xl" alt="icon" />
          )}
          <div className="min-w-0">
            ...
            <h2 className="text-lg font-medium text-slate-900 dark:text-slate-100 truncate leading-snug">
              {document}
            </h2>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <span className="inline-flex items-center gap-1.5 bg-primary/10 text-primary font-medium px-3 py-1.5 rounded-xl">
          <Clock className="h-3.5 w-3.5" />
          {formatDuration(totalDuration)} today
        </span>

        {videoId && channelName && (
          <span className="inline-flex items-center gap-1.5 font-medium px-3 py-1.5 rounded-xl border border-border bg-secondary/60 text-slate-600 dark:text-slate-300">
            {channelName}
          </span>
        )}

        {ytSession?.metadata?.category && (
          <span className="inline-flex items-center gap-1 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 font-semibold px-3 py-1.5 rounded-xl uppercase tracking-wide">
            {ytSession.metadata.category}
          </span>
        )}
      </div>
      ...
      {/* ── YouTube thumbnail ──────────────────────────────────── */}
      {videoId && (
        <div className="w-full overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 shadow-md relative group bg-black">
          <img
        ...
        <div className="w-full">
          ...
          <div className="relative w-full" style={{ paddingTop: '20px' }}>
            ...
            {LABEL_HOURS.map(hour => (
              <div
                key={hour}
                className="absolute top-0 text-[10px]  font-semibold text-slate-400 dark:text-slate-500 -translate-x-1/2"
            ...
            {/* Bar */}
            <div className="relative w-full h-10 bg-secondary/80 rounded-xl border border-border overflow-hidden shadow-inner">
          ...
          <div className="mt-4 flex flex-col gap-2 max-h-[220px] overflow-y-auto pr-1">
            {[...daySessions].reverse().map((session, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between gap-4 px-3 py-2 rounded-xl bg-accent/50 border border-border text-xs"
              >
                ...
                <span className="font-medium text-primary shrink-0">{formatDuration(session.duration_seconds)}</span>
              </div>
            ))}
          </div>
        </div>
      ) : (
        <div className="flex items-center justify-center h-28 rounded-xl border border-dashed border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 text-sm font-medium">
